...
- **2026-08-04**: 칠하는 방식(배경색/기호색) 전환 시 **칸 색과 기호 색을 맞바꾼다**. 두 모드는 같은 색을 어디에 칠하느냐의 차이인데, 모드만 뒤집으면 대비가 뒤집혀 기호가 배경에 묻힌다(흰 칸 + 흰 기호). 교환하면 흰 칸+검정 기호 ↔ 검정 칸+흰 기호로 자연스럽게 넘어가고 involution 이라 두 번 누르면 정확히 원상복귀. 모드와 두 색을 한 갱신으로 바꾸도록 `setViewOptions(patch)` 다중 키 setter 추가 — 따로 쓰면 중간 상태가 화면에 스친다
