...
- **2026-08-04**: 코바늘 코 바탕 크기 — **한 도안 안에서는 모두 같고**, 그 크기는 **그 도안에서 가장 긴 기호**를 따라간다. 코마다 다르면 배색이 들쭉날쭉해 덩어리로 읽히지 않고(대바늘 격자 칸과 같은 원리), 고정값으로 두면 긴뜨기 계열 도안에서 기호가 바탕 밖으로 삐져나온다. 세로만 그렇게 늘리고 가로는 코 간격의 절반(12px)으로 묶는다 — 가로까지 키우면 이웃 코의 바탕을 덮어 색 경계에서 뒤에 그린 쪽이 앞을 잘라먹는다
