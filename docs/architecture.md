...
- **2026-08-04**: 평면 배색 바탕이 **코가 차지한 칸 전체**를 칠하도록 — V(늘림)·A(줄임)는 한 기호가 2칸을 차지하는데(`visualClaim = max(produce, consume)`) 바탕은 1칸만 칠해 빈 칸이 생겼다. 레이아웃이 `PositionedStitch.cellSpan = { cells, offsetX }` 로 차지한 칸 수와 "기호에서 span 중심까지 거리"를 기록한다. 절대 좌표가 아니라 **오프셋**인 이유 — 기호는 정렬(L/R/C)에 따라 span 안 어느 칸에나 놓이고, 배치 후처리로 옮겨질 수도 있어서 오프셋이라야 바탕이 따라간다
