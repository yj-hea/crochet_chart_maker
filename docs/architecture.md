...
- **2026-08-04**: 색 일괄 지정·교체 뒤 **가장 많이 쓰인 색을 본문에서 빼고 칸 색(기본값)으로 내린다**(`withDominantColorAsDefault`). 안 그러면 모든 코에 `:색` 이 붙어 에디터가 색 동그라미로 뒤덮인다. 다수 색은 굳이 적을 필요가 없고 그려지는 결과도 같다. **색 없는 코가 하나라도 남아 있으면 정규화하지 않는다** — 그 상태에서 최다 색을 빼면 "원래 색이 없던 코" 와 "최다 색 코" 가 한 무리로 합쳐져 뜻이 달라진다. 타이핑 중에는 돌지 않고 버튼으로 하는 일괄 동작에서만 실행한다
