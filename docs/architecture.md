...
- **2026-08-04**: 코바늘 **원형** 코 바탕 크기 — 한 도안 안에서는 모두 같고, **한 칸 크기(24px)를 바닥으로** 도안에서 가장 긴 기호까지 세로로만 늘린다 (`rx = 12`, `ry = max(12, 가장 긴 기호 반높이 + 2.5)`). 기호 크기로만 잡으면 짧은뜨기 도안의 바탕이 칸의 절반(지름 12px)밖에 안 됐다. 코마다 다르면 배색이 들쭉날쭉해 덩어리로 읽히지 않고(대바늘 격자 칸과 같은 원리), 고정값으로 두면 긴뜨기 계열 도안에서 기호가 바탕 밖으로 삐져나온다. 세로만 그렇게 늘리고 가로는 코 간격의 절반(12px)으로 묶는다 — 가로까지 키우면 이웃 코의 바탕을 덮어 색 경계에서 뒤에 그린 쪽이 앞을 잘라먹는다
...
- **2026-08-04**: `ColorPalette` 의 "이 도안에서 쓴 색" 목록에 중복 제거 추가. 빈칸·칸·기호 색이 서로 같거나 본문 실 색과 겹치면 같은 값이 두 번 들어와 Svelte `{#each}` 키가 충돌했다 (`each_key_duplicate`) — 칸 색을 바꾼 뒤 배색을 다시 열면 터졌다
- **2026-08-04**: 코바늘 **평면**의 코 바탕은 격자 칸(rect)을 그대로 채운다 — 평면은 이미 사각 격자가 있으므로 대바늘과 같은 방식이 맞고, 칸끼리 딱 붙어 배색이 면으로 읽힌다. 타원은 격자가 없는 **원형**에만 쓴다. 분기는 `layout.gridGuide.type` (rect / concentric) 으로 — 렌더러가 도형 옵션을 따로 받지 않아도 된다
- **2026-08-04**: 평면 배색 바탕은 **기호가 놓인 한 칸만** 칠한다. V(늘림)·A(줄임)는 격자에서 2칸을 차지하지만(`visualClaim = max(produce, consume)`) 나머지 한 칸에는 실제로 코가 없다 — 거기까지 칠하니 비어야 할 칸이 물들었다. 차지한 칸 전체를 칠해 보려고 `PositionedStitch.cellSpan` 을 뒀다가 되돌렸다(그 필드도 함께 제거). 배색은 격자 점유가 아니라 **코가 있는 자리**를 따라가야 한다
- **2026-08-04**: 배색 목록을 **기본 배경색**(빈칸·칸)과 **배색**(기본 실 색·본문 실 색들) 두 묶음으로 나눔. `기호` 색은 "실 색을 지정하지 않은 코의 색"이므로 배경이 아니라 실 색 계열이고, 그래서 구분선 뒤 실 색들 앞에 놓고 코 수도 함께 보여준다. `칸` 색의 의미도 "색 없는 코의 배경" → "**기호가 있는 모든 칸**의 배경"으로 정정
- **2026-08-04**: `colorMode: 'auto'` 해석(코바늘 = 기호색, 대바늘 = 칸 채우기)을 `fillMode` 파생 스토어 하나로 모음. ChartViewer 가 같은 규칙을 따로 갖고 있었다
- **2026-08-04**: 기본 기호색을 단(`g.round`) 인라인 스타일 → **SVG 루트**로 옮김. 진행 하이라이트가 `g.style.color` 를 직접 지웠다 쓰는 구조라(Edit 모드에서는 매번 `''` 로 초기화) 같은 자리에 두면 기호색이 화면에 전혀 반영되지 않았다. 루트에 두면 상속으로 살아남고 하이라이트는 현재 단만 덮어쓰면 된다. 회귀 테스트로 "루트에 있고 `g.round` 에는 없다"를 고정
- **2026-08-04**: 칠하는 방식(배경색/기호색) 전환에서 **칸 색과 기호 색을 맞바꾸지 않는다**. 한 번 교환하도록 넣었다가 되돌렸다 — 두 값은 성질이 다르다. **칸 색은 실 색**(그 코가 무슨 실인지), **기호 색은 잉크 색**(선을 무슨 색으로 그리는지). 기호색 모드의 검정은 "실이 검정"이 아니라 도안을 검은 선으로 그린다는 관례일 뿐이라, 실 색 자리로 옮기면 흰 실이어야 할 코가 검정 칸이 된다. 그래서 실 색을 지정하지 않은 코는 두 모드에서 똑같이 보이고, 전환은 `:색` 을 적은 코의 표시만 바꾼다. (다중 키 setter `setViewOptions` 는 남겨 둠)
- **2026-08-04**: "색 없는 코에 실 색 일괄 지정"(`assignColorToUncolored`) 추가. 표시 옵션인 칸 색으로 색을 입히면 화면만 바뀌고 배색 목록에 실 색으로 잡히지 않아, 이후 일괄 교체 대상이 되지 못했다. 이 동작은 **본문에 `:색` 을 넣어** 정식 실 색으로 승격시키므로 그 뒤로는 다른 실 색과 똑같이 다뤄진다
- **2026-08-05**: 다수 색 자동 숨김(`withDominantColorAsDefault`) 취소. 다수 실 색을 `칸` 색 자리에 park 했는데, **칸 색은 기호색 모드의 종이색이지 실 색이 아니다** — 실 색을 종이색 칸에 넣으니 배색을 할당할 때마다 종이색이 바뀌었다. 실 색을 담을 자리를 새로 만드는 대신, 배색은 **본문에만** 산다는 원칙으로 되돌렸다
- **2026-08-05**: 배색 목록을 **표시 색**(빈칸·칸·기호)과 **배색**(본문 실 색들)으로 다시 나눔. `기호` 는 잉크라 실이 아니므로 표시 색 쪽이다. 배색 쪽 맨 앞의 **빈 동그라미 칩**이 아직 배색이 없는 코들이고, 색을 고르면 그 코들에 일괄로 `:색` 이 붙어 칩이 하나 생긴다 — "배색 원반이 없다가 하나 추가되는" 동작 그대로
- **2026-08-05**: 아이콘 폰트(`fa-fill-drip` 등)가 `?` 로 깨져 배색 할당 칩과 배경색/기호색 버튼의 아이콘을 CSS 도형으로 교체. 이 둘은 상태를 나타내는 표식이라 폰트 로딩에 기대지 않는 편이 낫다
